...
class Dashboard::CategoriesController < ApplicationController
  ...
  def update
    ...
    majorcategory = MajorCategory.find(major_category_id)
    
    wk_category_params = category_params
    wk_category_params[:major_category_name] = majorcategory.name
  ...
  private
    ...
    def category_params
      params.require(:category).permit(:name,:description,:major_category_name,:major_category_id, :image)
    end
end
